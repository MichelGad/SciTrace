...
def check_datalad():
    ...
    try:
        ...
        print("❌ DataLad not found or not working properly")
        print("Please install DataLad first: pip install datalad")
        return False

def create_demo_projects():
    """Create the demo projects with DataLad integration."""
    ...
    demo_projects = [
        ...
        {
            'name': 'Biomedical Clinical Data Analysis',
            'description': 'Analysis of patient clinical data including demographics, treatment outcomes, and statistical correlations. Features machine learning models for patient outcome prediction.',
            'research_type': 'biomedical',
            'dataset_spec': '4-6/2-3',
            'seed': 123
        },
        {
            'name': 'Computational Machine Learning Pipeline',
            'description': 'End-to-end machine learning pipeline for image classification. Includes data preprocessing, model training, hyperparameter tuning, and performance evaluation.',
            'research_type': 'computational',
            'dataset_spec': '2-4/3-5',
            'seed': 456
        }
    ]
    ...
    for i, project_config in enumerate(demo_projects, 1):
        print(f"\n🌱 Creating Demo Project {i}/3: {project_config['name']}")
        print("=" * 60)
...
def create_single_demo_project(project_config, project_num):
    ...
    with app.app_context():
        ...
        dataset_path = os.path.join(datalad_service.base_path, dataset_name)
        
        print(f"📁 Creating DataLad dataset at: {dataset_path}")
        
        # Create dataset using create-test-dataset
        dataset_info = datalad_service.create_dataset(
            dataset_path=dataset_path,
            name=project_config['name'],
            research_type=project_config['research_type']
        )
        
        print(f"     ✅ Dataset created with seed: {dataset_info['seed']}")
        ...
        # Create dataflow for visualization
        dataflow = create_demo_dataflow(project, project_config)
...
def create_demo_dataflow(project, project_config):
    """Create a demo dataflow for the project."""
...
def main():
    ...
    print("=" * 50)
    print("This script will create 3 realistic research projects with:")
    print("• DataLad datasets using create-test-dataset")
    print("• Sample research data and scripts")
    print("• Interactive dataflow visualizations")
    print("• Database entries for SciTrace")
    print()
    
    # Check prerequisites
    if not check_datalad():
        print("❌ Setup failed: DataLad not available")
        sys.exit(1)
    ...
    try:
        ...
        print("3. View your demo projects in the dashboard")
        ...
        print("\n💡 The demo projects include realistic research data and")
